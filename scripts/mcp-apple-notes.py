...
def cluster_notes(
    ...
    # Aggregate vectors for each note (average of all chunks)
    note_data = []
    ...
    vectors = np.array([note['vector'] for note in note_data])
    ...
    primary_labels = hdbscan_primary.fit_predict(vectors)
    ...
    if len(outlier_indices) == 0:
        ...
        for outlier_idx in outlier_indices:
            outlier_vector = vectors[outlier_idx].reshape(1, -1)
            ...
            for cluster_id, centroid in cluster_centroids.items():
                ...
                similarity = cosine_similarity(outlier_vector, centroid_reshaped)[0][0]
    ...
    if still_isolated > 0:
        ...
        if len(remaining_outliers) >= 2:
            remaining_vectors = vectors[remaining_outliers]
            ...
            secondary_labels = hdbscan_secondary.fit_predict(remaining_vectors)
    ...
    # between the note's vector and its assigned cluster centroid (0-1-ish).
    ...
    for cid in cluster_ids_final:
        ...
        if np.sum(mask) > 0:
            cluster_centroids[cid] = np.mean(vectors[mask], axis=0)
    ...
    for i in range(len(note_data)):
        ...
        if label != -1 and label in cluster_centroids:
            ...
            conf = float(cosine_similarity(vec, centroid)[0][0])
        else:
